"""
ULTIMATE V10: Fraud Detection БЕЗ LEAKAGE

ЧТО НОВОГО В V10:
1. УДАЛЕНЫ ВСЕ target_id фичи (100% leakage):
   - target_txn_count_past, target_fraud_rate_past_smooth
   - target_global_stats, unique_targets windows
   - is_new_target_for_client
2. Добавлены новые фичи БЕЗ утечки:
   - Rolling anomaly score (zscore_amount)
   - Behavioral fingerprint (hour deviation, activity profile)
   - Frequency encoding (OS/phone)
   - Client time density
   - EWM features
   - Rolling ratios
3. Добавлен CatBoost (3-модельный ансамбль)
4. Все фичи проверены на leakage
"""
...
def main():
    print("=" * 80)
    print("ULTIMATE V10 FRAUD DETECTION (NO LEAKAGE)")
    print("=" * 80)
