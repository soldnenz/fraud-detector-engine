...
def load_and_prepare():
    ...
    # --- 8. Фичи по получателю (target_id): история и fraud-rate ---
...
def train_random_ensemble(
    ...
    pos_weight = (len(y_train) - y_train.sum()) / (y_train.sum() + 1e-6)
    scale_pos_weight = pos_weight  # можно домножить на фактор, если захочешь
    print(f"Using scale_pos_weight = {scale_pos_weight:.3f}")
    ...
    precision_arr, recall_arr, thresholds = precision_recall_curve(y_val, val_pred_mean)
    f1_arr = 2 * precision_arr * recall_arr / (precision_arr + recall_arr + 1e-9)
    if len(thresholds) > 0:
        candidate_scores = f1_arr[:-1]
        best_idx = int(np.argmax(candidate_scores))
        ...
        best_threshold = 0.5
    print(f"\nEnsemble best threshold (validated): {best_threshold:.4f}")

    # Метрики на валидации
    y_val_pred = (val_pred_mean >= best_threshold).astype(int)
    val_auc = roc_auc_score(y_val, val_pred_mean)
    val_precision = precision_score(y_val, y_val_pred, zero_division=0)
    val_recall = recall_score(y_val, y_val_pred, zero_division=0)
    val_f1 = f1_score(y_val, y_val_pred, zero_division=0)
    print(f"[VAL ENSEMBLE]  ROC-AUC: {val_auc:.3f} | P: {val_precision:.3f} | R: {val_recall:.3f} | F1: {val_f1:.3f}")

    # Метрики на тесте
    y_test_pred = (test_pred_mean >= best_threshold).astype(int)
    test_auc = roc_auc_score(y_test, test_pred_mean)
    test_precision = precision_score(y_test, y_test_pred, zero_division=0)
    test_recall = recall_score(y_test, y_test_pred, zero_division=0)
    test_f1 = f1_score(y_test, y_test_pred, zero_division=0)
    print(f"[TEST ENSEMBLE] ROC-AUC: {test_auc:.3f} | P: {test_precision:.3f} | R: {test_recall:.3f} | F1: {test_f1:.3f}")
...
def main():
    ...
    # ===== Variant 3: Time-based, stronger model, no weight =====
