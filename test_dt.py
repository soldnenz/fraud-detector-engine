...
SAMPLE_FRACTION = 0.50  # Use 15% of data for faster training (~1M transactions)
...
def train_ensemble(
    ...
    return all_models, val_metrics, test_metrics, weights, best_threshold
...
# ------------------------------------------------------------------------------
# Cross-dataset validation на transactions.csv
# ------------------------------------------------------------------------------

def load_and_prepare_transactions_for_test(
    feature_names: List[str]
) -> Tuple[pd.DataFrame, pd.Series, pd.Series]:
    """
    Загружает transactions.csv и адаптирует фичи под обученную модель.
    Создает те же фичи, что и для PaySim (насколько возможно).
    """
    print("\n" + "=" * 80)
    print("LOADING EXTERNAL TEST DATASET: transactions.csv")
    print("=" * 80)
    ...
    df_transactions["trans_date"] = _parse_date(df_transactions["trans_date"]).dt.date
    df_transactions["trans_datetime"] = _parse_date(df_transactions["trans_datetime"])
    df_behavior["trans_date"] = _parse_date(df_behavior["trans_date"]).dt.date
    ...
    print(f"Fraud count: {data['label'].sum():,}")
    
    # --- Create PaySim-like features ---
    print("\nAdapting features to PaySim format...")
    
    X = pd.DataFrame(index=data.index)
    
    # 1. Transaction type (все будут TRANSFER, так как у нас только переводы)
    X['type_CASH_OUT'] = 0
    X['type_PAYMENT'] = 0
    X['type_CASH_IN'] = 0
    X['type_TRANSFER'] = 1  # Все транзакции - переводы
    X['type_DEBIT'] = 0
    
    # 2. Amount features
    X['amount'] = data['amount'].fillna(0)
    X['log_amount'] = np.log1p(X['amount'])
    X['amount_squared'] = X['amount'] ** 2
    
    # 3. Balance features (мы не знаем балансов, используем заглушки)
    # Модель должна полагаться на другие фичи
    X['balance_orig_ratio'] = 1.0
    X['balance_dest_ratio'] = 1.0
    X['balance_orig_change'] = 0.0
    X['balance_dest_change'] = 0.0
    X['error_balance_orig'] = 0.0
    X['error_balance_dest'] = 0.0
    
    # 4. Zero balance flags (заглушки)
    X['orig_zero_balance'] = 0
    X['dest_zero_balance'] = 0
    X['orig_zero_after'] = 0
    X['dest_zero_after'] = 0
    
    # 5. Amount flags
    X['amount_equals_old_balance'] = 0
    X['amount_greater_balance'] = 0
    
    # 6. Customer-based features
    data_sorted = data.sort_values(['cst_id', 'trans_datetime']).reset_index(drop=True)
    customer_groups = data_sorted.groupby('cst_id', group_keys=False)
    
    X['cust_txn_count'] = customer_groups.cumcount()
    
    cust_amount_cumsum = customer_groups['amount'].cumsum() - data_sorted['amount']
    X['cust_amount_mean'] = cust_amount_cumsum / (X['cust_txn_count'] + 1)
    X['cust_amount_mean'] = X['cust_amount_mean'].fillna(data['amount'].mean())
    
    X['amount_vs_cust_mean'] = X['amount'] / (X['cust_amount_mean'] + 1)
    X['amount_diff_cust_mean'] = X['amount'] - X['cust_amount_mean']
    
    # Time since last transaction (используем timestamp если есть)
    if 'trans_datetime' in data.columns:
        prev_datetime = customer_groups['trans_datetime'].shift(1)
        hours_since = (data_sorted['trans_datetime'] - prev_datetime).dt.total_seconds() / 3600
        X['hours_since_prev'] = hours_since.fillna(999)
    else:
        X['hours_since_prev'] = 999
    
    # 7. Destination-based features
    data_sorted_dest = data.sort_values(['target_id', 'trans_datetime']).reset_index(drop=True)
    dest_groups = data_sorted_dest.groupby('target_id', group_keys=False)
    
    X['dest_txn_count'] = dest_groups.cumcount()
    dest_fraud_cumsum = dest_groups['label'].cumsum() - data_sorted_dest['label']
    
    X['dest_fraud_rate'] = dest_fraud_cumsum / (X['dest_txn_count'] + 1)
    X['dest_fraud_rate'] = X['dest_fraud_rate'].fillna(0)
    
    alpha = 5.0
    global_fraud_rate = data['label'].mean()
    X['dest_fraud_rate_smooth'] = (
        dest_fraud_cumsum + alpha * global_fraud_rate
    ) / (X['dest_txn_count'] + alpha)
    
    # 8. Interaction features
    X['amount_x_dest_fraud'] = X['amount'] * X['dest_fraud_rate_smooth']
    X['amount_x_zero_dest'] = X['amount'] * X['dest_zero_balance']
    X['amount_x_error_orig'] = X['amount'] * X['error_balance_orig'].abs()
    X['cashout_x_amount'] = X['type_CASH_OUT'] * X['log_amount']
    X['transfer_x_amount'] = X['type_TRANSFER'] * X['log_amount']
    
    # 9. High-risk flags
    X['is_high_amount'] = (X['amount'] > X['amount'].quantile(0.95)).astype(int)
    X['is_round_amount'] = (X['amount'] % 1000 == 0).astype(int)
    
    # 10. Merchant flag (target_id как аналог nameDest)
    X['dest_is_merchant'] = 0  # У нас нет информации о мерчантах
    
    # Ensure all required features exist
    for feat in feature_names:
        if feat not in X.columns:
            X[feat] = 0
            print(f"  Added missing feature: {feat} = 0")
...
def test_on_external_dataset(
    ...
    """
    Тестирует обученную модель на внешнем датасете transactions.csv
    """
    print("\n" + "=" * 80)
    print("CROSS-DATASET VALIDATION: Testing on transactions.csv")
    print("=" * 80)
    
    try:
        # Load and prepare external test data
        X_external, y_external, amounts_external = load_and_prepare_transactions_for_test(
            feature_names
        )
        ...
        # LGBM models
        for i, model in enumerate(models["lgbm"]):
            proba = model.predict(X_external, num_iteration=model.best_iteration)
            all_probas.append(proba)
            print(f"  LGBM model {i}: done")
        
        # XGB models
        for i, model in enumerate(models["xgb"]):
            dtest = xgb.DMatrix(X_external)
            proba = model.predict(dtest, iteration_range=(0, model.best_iteration))
            all_probas.append(proba)
            print(f"  XGB model {i}: done")
        
        # Weighted ensemble prediction
        y_proba_ensemble = sum(w * p for w, p in zip(weights, all_probas))
        
        # Evaluate with the same threshold from training
        print(f"\nUsing threshold from training: {threshold:.4f}")
        
        metrics_external = evaluate_predictions(
            y_external, 
            y_proba_ensemble, 
            threshold, 
            amounts_external
        ...
        print("\n" + "=" * 80)
        print("EXTERNAL TEST RESULTS (transactions.csv)")
        print("=" * 80)
        print_metrics("EXTERNAL TEST", metrics_external)
        
        # Also try optimal threshold for this dataset
        threshold_optimal = pick_best_threshold_by_f1(y_external, y_proba_ensemble)
        if abs(threshold_optimal - threshold) > 0.05:
            print(f"\nOptimal threshold for this dataset: {threshold_optimal:.4f}")
            metrics_optimal = evaluate_predictions(
                y_external, 
                y_proba_ensemble, 
                threshold_optimal, 
                amounts_external
            )
            print_metrics("EXTERNAL TEST (optimal thr)", metrics_optimal)
        
        return metrics_external
        
    except Exception as e:
        print(f"\n⚠️  Error testing on external dataset: {e}")
...
def main():
    ...
    ensemble_models, ens_val, ens_test, ensemble_weights, ensemble_threshold = train_ensemble(
        X_train=X_train,
        y_train=y_train,
        X_val=X_val,
        y_val=y_val,
    ...
    # 6. Test on external dataset (transactions.csv)
    print("\n" + "=" * 80)
    print("PHASE 3: Cross-Dataset Validation")
    print("=" * 80)
    
    external_metrics = test_on_external_dataset(
        models=ensemble_models,
    ...
    print("\n💡 Key Insights:")
    print(f"  Training dataset: {len(df):,} transactions")
    print(f"  Fraud rate: {y.mean():.4f}")
    print(f"  Best single AUC: {max(lgbm_test.roc_auc, xgb_test.roc_auc):.4f}")
    print(f"  Ensemble AUC (PaySim test): {ens_test.roc_auc:.4f}")
    print(f"  Ensemble F1 (PaySim test): {ens_test.f1:.4f}")
    if ens_test.business_metrics:
        bm = ens_test.business_metrics
        print(f"  Fraud prevention (PaySim): {bm.fraud_prevention_rate*100:.1f}%")
        print(f"  Money saved (PaySim): ${bm.blocked_fraud_amount:,.0f}")
    
    if external_metrics:
        print(f"\n  🌍 CROSS-DATASET PERFORMANCE:")
        print(f"  External AUC (transactions.csv): {external_metrics.roc_auc:.4f}")
        print(f"  External F1 (transactions.csv): {external_metrics.f1:.4f}")
        print(f"  External Precision: {external_metrics.precision:.3f}")
        print(f"  External Recall: {external_metrics.recall:.3f}")
        if external_metrics.business_metrics:
            bm_ext = external_metrics.business_metrics
            print(f"  External fraud prevention: {bm_ext.fraud_prevention_rate*100:.1f}%")
            print(f"  External money saved: ${bm_ext.blocked_fraud_amount:,.0f}")
